...
def maximum_Sharpe(means, cov):
    ...
    return np.array(y.value.flat).round(4), expected_return.value, expected_vol.value
...
#y,expected_mean,expected_vol,k,weights = maximum_Sharpe(mu_excess, Sigma)
weights,expected_mean,expected_vol = maximum_Sharpe(mu_excess, Sigma)
...
leverage = weight_matrix.abs()
leverage = leverage.sum(axis=1)
weight_matrix = weight_matrix.divide(leverage, axis=0)
...
while start_portfolio_plus1 <= end_date:
    ...
    if rebalancing_day_check % rebalancing_period == 0 :
        ...
        # Constraint the weights to have leverage of 1
        leverage = weights.abs()
        leverage = leverage.sum(axis=1)
        weights = weights.divide(leverage, axis=0)

        weight_matrix = [weight_matrix, weights]
        weight_matrix = pd.concat(weight_matrix, ignore_index=False)

        df_length = len(weighted_returns)
        weighted_returns.loc[df_length] = ms
...
weighted_returns.to_excel(r'/Users/gmmtakane/Desktop/Thesis/weighted_returns msqp n' + '.xlsx', index = True)
weight_matrix.to_excel(r'/Users/gmmtakane/Desktop/Thesis/weights msqp n ' + '.xlsx')
returns.to_excel(r'/Users/gmmtakane/Desktop/Thesis/log returns msqp n' + '.xlsx')
